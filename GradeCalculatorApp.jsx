...
/* ============================================================
   [GEMINI CONFIG] — AI ช่วยวิเคราะห์แนวโน้มสายอาชีพ (ไม่บังคับ)
   ------------------------------------------------------------
   1) ขอ API key ฟรีที่ https://aistudio.google.com/apikey
   2) ⚠️ สำคัญ: ไปที่ Google Cloud Console → Credentials → เปิดคีย์นั้น
      → Application restrictions = "Websites (HTTP referrers)"
        ใส่โดเมนของคุณ เช่น  https://qf6rrqnymb-dev.github.io/*
      → API restrictions = "Generative Language API"
      (เพราะคีย์จะโผล่ในหน้าเว็บ · การจำกัดโดเมน + free tier ช่วยกันคนอื่นแอบใช้)
   3) วางคีย์ด้านล่าง แล้วรัน python3 build.py
   ปล่อยว่าง = ปิดฟีเจอร์ AI (ใช้การวิเคราะห์ heuristic อย่างเดียว)
   ============================================================ */
const GEMINI_API_KEY = ""; // ← วางคีย์ที่นี่ (ดู README · ต้องเป็นคีย์ที่มีโควตา)
const GEMINI_MODEL = "gemini-2.0-flash"; // เปลี่ยนได้: gemini-flash-latest ฯลฯ (gemini-2.5-flash บล็อกบัญชีใหม่)
...
async function callGemini(prompt) {
  ...
  if (!text.trim()) throw new Error("Gemini ตอบกลับว่าง ลองใหม่อีกครั้ง");
  return text.trim();
}
